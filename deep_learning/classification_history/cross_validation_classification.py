from hyperparameter_tuning_summary import get_best_hyperparameter, get_best_parameter
...
X_train, _, y_train, _ = get_split_dataset(regressor=False, min_max_scaler=False)

classifier = DeepClassifierModel(input_dim=X_train.shape[1])
#
best_hyperparameters = get_best_parameter()[0].values  # get the best hyperparams from the tuning process done in
...
def create_model(best_hyperparameter=best_hyperparameters):
    ...
    model.compile(optimizer=classifier.set_optimizer(best_hyperparameter['optimizer'],
